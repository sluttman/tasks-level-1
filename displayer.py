...
class TreeDisplayer:
    ...
    def ask_for_json(self):
        ...
        while invalid_response:
            user_decision = input(
                f"What is the name of the json file you want to display components from? (e.g. productfamily.json write productfamily)\n"
            )
    ...
    def display_one_product_or_kit(self, json_tree, indentation):
        ...
        for i in range(0, indentation):
            indent = indent + "         "
        print(
            f"""{indent}{json_tree.get("name")}, Price: {json_tree.get("price")}, Type: {json_tree.get("type")}\n"""
        )
